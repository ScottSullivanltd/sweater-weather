...
RSpec.describe RoadtripsFacade do
  describe "class methods" do
    ...
    xit ".hourly_weather_temp", :vcr do
      time = [1, 20, 15]
      ...
      weather = RoadtripsFacade.hourly_weather_temp(data, time)

      expect(weather).to eq(80.7)
    end

    xit ".hourly_weather_conditions", :vcr do
      time = [1, 20, 15]
      allow(RoadtripsFacade).to receive(:get_travel_time).and_return(time)
      ...
      weather = RoadtripsFacade.hourly_weather_conditions(data, time)
